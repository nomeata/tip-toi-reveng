...
ppCommand _ t xs (Rand r v)      = printf "T(%s,%s)" (ppReg r) (ppTVal v)
ppCommand _ t xs (ArmTimer m)    = printf "AT(%d)" m
ppCommand _ t xs CancelTimer     = printf "CT"
ppCommand _ t xs (SoundProfile p (Const 0)) = printf "SP(%d)" p
ppCommand _ t xs (SoundProfile p v)         = printf "SP(%d,%s)" p (ppTVal v)
...
ppCommonGame t g =
    ...
    (gEarlyRounds g)
    (gRepeatOID g)
    (gTuningX g) (gTuningW g) (gTuningV g)
